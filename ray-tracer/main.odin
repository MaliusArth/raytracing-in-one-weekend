...
make_metallic_data :: proc(albedo: color, fuzz: f64) -> material_data {
	...
	return material_data{albedo=albedo, param1=fuzz}
}
...
material_scatter :: proc(material: material, ray_in: ^ray, hit: ^hit_record) -> (ray, color, bool) {
...
ray_cast :: proc(r: ^ray, max_ray_bounces: i64, spheres: []sphere, materials: []material) -> color {
	...
	for _ in 0..=max_ray_bounces {
		...
		if closest_t < math.F64_MAX {
			...
			// TODO: instead of including material types in the materials array maybe encode the info in the sphere closest_sphere.material_index; closest_sphere.material_type; (could even be shifted into the material_index)
...
render :: proc(render_target: image, camera: camera, materials: []material, spheres : []sphere, $print_progress : bool) {
	...
	// view-space
	pixel_deltas_vs := view_plane_half_size * 2 / camera.image_size * vec2{1, -1} // vertical flip
	...
	// view-space
	view_plane_top_left_pixel_center_vs := view_plane_half_size * vec2{-1, 1} + pixel_deltas_vs * 0.5
...
build_dev_scene :: proc(allocator := context.allocator) -> (camera: camera, materials: [dynamic]material, spheres: #soa[dynamic]sphere) {
